...
function showForce(strength){
    ...
    if ((strength < 12)){
        document.getElementById("errorPassForce").innerHTML = '<span>Digite uma senha forte, incluindo carateres <br> especiais como: <strong>"@#$%&*"<strong></span>';
    }else if(strength < 38){
...
function verifyPass(){
    ...
    var confirm_pass = formuser.confirm_pass.value;
            ...
            if(pass == '' || pass.length <= 7){
                document.getElementById('congrats').innerHTML = "<span style='color: #ffffff'>confirme a senha acima</span>"
                formuser.pass.focus();
                return false;
            }
